...
拿到用户请求后，在选择工具前先做一次站点经验匹配：

```bash
node "$CLAUDE_SKILL_DIR/scripts/match-site.mjs" "用户原始请求"
```

- 若命中，必须先吸收输出的站点经验，再决定入口、工具和操作路径。
- 若未命中，按通用流程继续，不要为了“套经验”而强行匹配站点。
- 当宿主环境未注入 `CLAUDE_SKILL_DIR` 时，改用 skill 绝对路径执行同一脚本。
